...
//            Route::resource('template/pdf', 'PdfTemplatesController', array('only' => array('index','show', 'store', 'update') ));
...
//    only call below command when new project cloned first time
//    Artisan::call('storage:link');
    return "Cleared!";
});
Route::get('/stripe-create-intent-transaction','ClientArea\HomeController@test')->name('stripe.intent');
Route::get('/save-stripe-transaction','ClientArea\CheckoutController@stripeSuccess2')->name('save-stripe-transaction');

Route::get('/cron/invoices-subscription', function () {
    Artisan::call('invoicer:recurring-invoices');
    return response()->json('Invoices Cron executed');
});